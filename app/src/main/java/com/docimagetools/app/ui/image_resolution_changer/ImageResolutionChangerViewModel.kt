...
class ImageResolutionChangerViewModel : ViewModel() {
    ...
    protected val getOriginalAspectRatio: Double
        protected get() {
            ...
            Log.i(
                "Resolution Changer",
                "Aspect Ratio: ${(resolution[0]!!).toDouble() / resolution[1]!!.toDouble()}"
            )
    ...
    suspend fun resize(context: Context, file: File): File {
        ...
        val resizedImageFile = Compressor.compress(context, file) {

            resolution(getSelectedWidth, getSelectedHeight)
            format(Bitmap.CompressFormat.JPEG)
        }
        Log.i("Compressor", "Original   File Size: ${file.length()}")
        Log.i("Compressor", "Compressed File Size: ${resizedImageFile.length()}")
        Log.i(
            "Compressor",
        ...
        Log.i(
            "Compressor",
            "Compressed File Resolution: ${getImageSize(resizedImageFile.toURI()).joinToString()}"
        )

        resizedImage = resizedImageFile
